...
(async function test(username, password) {
  ...
  try {
    ...
    const { key, userid, token } = result.data;
    const messageId = md5([randomString(16, unix_time)].join(""));
    const sign = md5([randomString(messageId, key, unix_time)].join(""));
    console.log({
      userid,
      key,
      token,
